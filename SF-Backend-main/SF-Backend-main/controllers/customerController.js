...
const createCustomer = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const customer = fallbackStore.createCustomer({
        ...
        kycDocumentType: normalizeKycDocumentType(kycDocumentType),
        kycDocumentImage,
...
const getCustomers = async (req, res, next) => {
  try {
    if (shouldUseFallback(req)) {
      return res.status(200).json(fallbackStore.listCustomers());
    }
...
const getCustomerById = async (req, res, next) => {
  try {
    if (shouldUseFallback(req)) {
      const customer = fallbackStore.findCustomerById(req.params.id);
      if (!customer) {
...
const updateCustomer = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const customer = fallbackStore.updateCustomer(req.params.id, updates);
      if (!customer) {
        return res.status(404).json({ message: "Customer not found" });
      }
...
const deleteCustomer = async (req, res, next) => {
  try {
    if (shouldUseFallback(req)) {
      const deleted = fallbackStore.deleteCustomer(req.params.id);
      if (!deleted) {
        return res.status(404).json({ message: "Customer not found" });
      }
