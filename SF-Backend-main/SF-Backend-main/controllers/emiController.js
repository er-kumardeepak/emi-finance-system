...
const getEmiSchedule = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const schedule = fallbackStore.listInstallmentsForLoan(loanId);
      return res.status(200).json({ loanId, schedule });
    }
...
const payEmi = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const installment = fallbackStore.payInstallment(installmentId, req.user.id, modeToSave);
      if (!installment) {
        return res.status(404).json({ message: "Installment not found" });
      }
      return res.status(200).json({ message: "Installment marked as paid", installment });
    }
