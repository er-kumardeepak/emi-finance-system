...
const createLoan = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const customer = fallbackStore.findCustomerById(customerId);
      if (!customer) {
        return res.status(404).json({ message: "Customer not found" });
      }

      const purchase = purchaseDate ? new Date(purchaseDate) : new Date();
      const loan = fallbackStore.createLoan({
        customerId,
        ...
        loanAmount: Number(productPrice) - Number(downPayment) + charge,
        emiPlan: plan,
        monthlyEmi: Number(((Number(productPrice) - Number(downPayment) + charge) / plan).toFixed(2)),
        purchaseDate: purchase,
        createdBy: req.user.id,
      });

      const firstDueDate = addMonths(purchase, 1);
      fallbackStore.createInstallments(loan._id, customerId, plan, loan.monthlyEmi, firstDueDate);
...
const getLoans = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      return res.status(200).json(fallbackStore.listLoans());
    }
...
const getLoanById = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const loan = fallbackStore.findLoanById(id);
      if (!loan) {
...
const getLoanInstallments = async (req, res, next) => {
  try {
    ...
    if (shouldUseFallback(req)) {
      const loan = fallbackStore.findLoanById(loanId);
      if (!loan) {
        return res.status(404).json({ message: "Loan not found" });
      }

      const installments = fallbackStore.listInstallmentsForLoan(loanId);
