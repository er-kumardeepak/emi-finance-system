...
const ReportSection = ({ title, icon: Icon, children }) => (
    <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-soft dark:border-slate-700/80 dark:bg-slate-900 sm:p-6">
        <div className="mb-4 flex items-center gap-2.5 sm:mb-5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-sky-50 text-sky-600 dark:bg-sky-900/30 dark:text-sky-400 sm:h-11 sm:w-11">
...
const StatCard = ({ label, value, type = 'number', highlight = false }) => (
    <div className={`rounded-xl p-3 sm:p-4 ${highlight ? 'bg-sky-50 dark:bg-sky-900/20' : 'bg-slate-50 dark:bg-slate-800/50'}`}>
        <p className={`text-xs font-medium sm:text-sm ${highlight ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-400'}`}>{label}</p>
        <p className={`mt-1 text-lg font-bold sm:mt-2 sm:text-2xl ${highlight ? 'text-sky-700 dark:text-sky-300' : 'text-slate-900 dark:text-white'}`}>
        ...
        <div className="space-y-6 pb-12">
            ...
            <ReportSection title="4. Customer Report" icon={FiUsers}>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <StatCard label="With Active Loans" value={data.customer.customersWithActiveLoans} />
                    <StatCard label="With Overdue EMIs" value={data.customer.customersWithOverdueEmis} highlight />
                </div>
            </ReportSection>
